...
export default function PaceApp() {
  ...
  async function logout() { setBusy(true); try { await account.deleteSession({ sessionId: "current" }); setUser(null); setTasks([]); setShowTools(false); setShowSettings(false); } catch (error) { setNotice(cleanError(error)); } finally { setBusy(false); } }
  ...
  async function resetWorkspace() {
    ...
    setBusy(true); try { for (const task of tasks) await tablesDB.deleteRow({ databaseId, tableId: tasksTableId, rowId: task.id }); setTasks([]); setSettings(DEFAULT_SETTINGS); setShowTools(false); setShowSettings(false); } catch (error) { setNotice(cleanError(error)); } finally { setBusy(false); }
  }
  ...
  function importData(file: File) { const reader = new FileReader(); reader.onload = async () => { try { const data = JSON.parse(String(reader.result)); if (!Array.isArray(data.tasks) || !user) throw new Error("Invalid PACE workspace"); for (const raw of data.tasks) { const task = raw as Task; if (!task.title || !task.due) continue; await tablesDB.createRow({ databaseId, tableId: tasksTableId, rowId: ID.unique(), data: { userId: user.$id, title: String(task.title).slice(0, 255), subject: String(task.subject || "General").slice(0, 100), dueDate: `${task.due}T23:59:00.000Z`, estimatedMinutes: Math.max(5, Number(task.minutes) || 5), difficulty: "medium", completed: Boolean(task.done), priority: (task.priority || "Medium") as Priority, type: (task.type || "Other") as TaskType }, permissions: [Permission.read(Role.user(user.$id)), Permission.update(Role.user(user.$id)), Permission.delete(Role.user(user.$id))] }); } await loadTasks(user); setSettings({ ...DEFAULT_SETTINGS, ...(data.settings || {}) }); setShowTools(false); } catch (error) { setNotice(cleanError(error)); } }; reader.readAsText(file); }
  ...
  return <main>
    ...
    <footer><span>PACE / Academic workload intelligence</span><button className="footer-account" onClick={() => setShowTools(v => !v)}>Account · {user.name || user.email}</button><span className="footer-credit">Created by Koglesh R. Murugan · Build. Learn. Ship.</span></footer>
    {showTools && <div className="workspace-pop" role="dialog" aria-label="PACE account menu"><div className="panel-label">PACE MENU</div><div className="menu-account"><strong>{user.name || "PACE student"}</strong><span>{user.email}</span></div><div className="menu-section"><span className="menu-section-title">ACCOUNT</span><button onClick={() => { setShowTools(false); setShowSettings(true); }}>⚙ Settings</button><button onClick={logout}>⇄ Switch account</button><button onClick={logout}>↪ Log out</button></div><div className="menu-section"><span className="menu-section-title">DATA</span><button onClick={exportData}>↓ Export workspace</button><label className="file-button">↑ Import workspace<input type="file" accept="application/json,.json" onChange={e => { const file = e.target.files?.[0]; if (file) importData(file); e.currentTarget.value = ""; }} /></label></div><div className="menu-section danger-section"><span className="menu-section-title">DANGER ZONE</span><button className="danger-button" onClick={resetWorkspace}>Reset workspace</button></div></div>}
    {showSettings && <div className="modal-backdrop" role="presentation" onMouseDown={e => { if (e.target === e.currentTarget) setShowSettings(false); }}><div className="modal settings-modal" role="dialog" aria-modal="true" aria-labelledby="settings-title"><button className="close" aria-label="Close settings" onClick={() => setShowSettings(false)}>×</button><span className="kicker">PACE / SETTINGS</span><h2 id="settings-title">Plan around real life.</h2><p>These controls shape how PACE distributes your workload. They are saved separately for your account.</p><div className="settings-block"><div className="settings-title"><div><strong>Daily capacity</strong><span>Maximum study time on normal days.</span></div><b>{settings.dailyCapacity} min</b></div><input aria-label="Daily capacity setting" type="range" min="30" max="360" step="15" value={settings.dailyCapacity} onChange={e => setSettings(s => ({ ...s, dailyCapacity: Number(e.target.value) }))} /></div><div className="settings-grid"><label>Weekend capacity<input type="number" min="0" max="720" step="15" value={settings.weekendCapacity} onChange={e => setSettings(s => ({ ...s, weekendCapacity: Math.max(0, Number(e.target.value) || 0) }))} /><span>Minutes available on Saturday and Sunday.</span></label><label>Break between blocks<input type="number" min="0" max="60" step="5" value={settings.breakMinutes} onChange={e => setSettings(s => ({ ...s, breakMinutes: Math.max(0, Number(e.target.value) || 0) }))} /><span>Planning preference for future session grouping.</span></label></div><label className="settings-toggle"><input type="checkbox" checked={settings.schoolDaysOnly} onChange={e => setSettings(s => ({ ...s, schoolDaysOnly: e.target.checked }))} /><span><strong>School days only</strong><small>Skip weekends when building the seven-day plan.</small></span></label><div className="settings-summary"><span>ACCOUNT</span><strong>{user.name || user.email}</strong><small>Changes save automatically to this account.</small></div><button className="primary full" onClick={() => setShowSettings(false)}>Done · Save settings</button></div></div>}
